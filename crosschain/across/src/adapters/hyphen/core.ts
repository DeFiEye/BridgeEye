...
import { DataCache, nameToChainId }from '../../utils';
...
export async function calculateBridgeFee(
  ...
  const fromChain = networks.find(
    (a: any) =>
      a.name === fromChainName ||
      (formChainIdTranslated && a.chainId == formChainIdTranslated)
  );

  // console.log("toChainIdTranslated", toChainIdTranslated);
  const toChain = networks.find(
    (a: any) =>
      a.name === toChainName ||
      (toChainIdTranslated && a.chainId == toChainIdTranslated)
  );
  ...
  return {
    ...
    token: {
      symbol: fromChainToken.symbol,
      decimals: fromChainToken.decimal,
      mainnetAddress: fromChainToken.address,
    },
    outputToken: {
      symbol: toChainToken.symbol,
      decimals: toChainToken.decimal,
      mainnetAddress: toChainToken.address,
    },
    fee: totalFee,
    // percent
    breakdown,
